...
const ASSET_CACHE_VERSION = "20260709debatejoin";
...
const IS_DESKTOP_APP = Boolean(window.WSC_DESKTOP_APP);
...
const CAMPUS_FEEDBACK_EMAIL = "support@wscapp.app";
...
const createProgressStorageController = window.WSC_CREATE_PROGRESS_STORAGE_CONTROLLER || null;
const appDomService = window.WSC_APP_DOM_SERVICE || null;
const createDevicePresentationController = window.WSC_CREATE_DEVICE_PRESENTATION_CONTROLLER || null;
const createAppSettingsController = window.WSC_CREATE_APP_SETTINGS_CONTROLLER || null;
...
let devicePresentationController = null;
...
const appLifecycleController = window.WSC_APP_LIFECYCLE_CONTROLLER.create({
  ...
  live: {
    service: alpacapardyLiveSupabaseService,
    getSupabaseClient,
    isHost: () => getAlpacapardyLiveIdentityContext().isHost,
...
setupDevicePresentationController();
...
function scheduleModeChoiceCardSpread(column, board) {
  ...
  if (isTouchLandscapePresentationActive()) {
    ...
    column.classList.add("is-open");
    column.classList.remove("is-closing");
    applyTouchModeChoiceCardGrid(column);
    return;
  }
...
function applyTouchModeChoiceCardGrid(column) {
  const grid = column.querySelector(".mode-choice-card-grid");
  if (!grid) {
    return;
  }

  const compactTouchLayout = window.innerWidth <= 980;
  const stackedCompactTouchLayout = compactTouchLayout && window.innerWidth < 620;
  const wideTouchLayout = window.innerWidth >= 1180;
  const compactCardBlockSize = "clamp(46px, 13vh, 54px)";
  const cardInlineSize = wideTouchLayout
    ? "clamp(170px, 15vw, 210px)"
    : compactTouchLayout
      ? stackedCompactTouchLayout ? "min(260px, calc(100% - 4px))" : "100%"
    : "clamp(132px, 16vw, 180px)";

  grid.style.setProperty("position", "absolute", "important");
  grid.style.setProperty(
    "inset",
    compactTouchLayout ? "0 6px 6px" : wideTouchLayout ? "64px 14px 14px" : "58px 10px 10px",
    "important"
  );
  grid.style.setProperty("display", "grid", "important");
  grid.style.setProperty(
    "grid-template-columns",
    stackedCompactTouchLayout ? "minmax(0, 1fr)" : wideTouchLayout ? "repeat(3, minmax(0, 1fr))" : "repeat(2, minmax(0, 1fr))",
    "important"
  );
  grid.style.setProperty("grid-auto-rows", compactTouchLayout ? compactCardBlockSize : "auto", "important");
  grid.style.setProperty("gap", compactTouchLayout ? "6px" : wideTouchLayout ? "10px" : "8px", "important");
  grid.style.setProperty("overflow-y", compactTouchLayout ? "hidden" : "auto", "important");
  ...
  grid.querySelectorAll(".wizard-choice-card").forEach((card) => {
    card.style.setProperty("position", "relative", "important");
    card.style.setProperty("left", "auto", "important");
    card.style.setProperty("top", "auto", "important");
    card.style.setProperty("grid-column", stackedCompactTouchLayout ? "1 / -1" : "auto", "important");
    card.style.setProperty("grid-row", "auto", "important");
    card.style.setProperty("inline-size", cardInlineSize, "important");
    card.style.setProperty("width", cardInlineSize, "important");
    card.style.setProperty("max-inline-size", cardInlineSize, "important");
    card.style.setProperty("max-width", cardInlineSize, "important");
    card.style.setProperty("min-width", "0", "important");
    card.style.setProperty("height", compactTouchLayout ? compactCardBlockSize : "auto", "important");
    card.style.setProperty("min-height", compactTouchLayout ? compactCardBlockSize : wideTouchLayout ? "64px" : "58px", "important");
    card.style.setProperty("max-height", compactTouchLayout ? compactCardBlockSize : "none", "important");
    card.style.setProperty("padding", compactTouchLayout ? "5px 7px" : wideTouchLayout ? "8px 10px" : "8px 10px", "important");
    card.style.setProperty("justify-self", stackedCompactTouchLayout ? "center" : "stretch", "important");
...
function isTouchLandscapePresentationActive() {
...
function revealTouchModeChoiceMenu(column) {
  ...
  const reveal = () => {
    ...
    if (board) {
      ...
      const centeredLeft = board.scrollLeft + columnRect.left - boardRect.left - ((board.clientWidth - columnRect.width) / 2);
      board.scrollTo({ left: Math.max(0, centeredLeft), behavior: "auto" });
    }

    const target = column.querySelector(".mode-choice-card-grid") || column;
    const routeBuilder = column.closest("#routeBuilder");
    const viewportHeight = window.visualViewport?.height || window.innerHeight;
    const routeRect = routeBuilder?.getBoundingClientRect();
    const targetRect = target.getBoundingClientRect();
    const topPadding = 8;
    const bottomPadding = 8;
    if (routeRect && routeRect.top > topPadding) {
      window.scrollBy({ top: routeRect.top - topPadding, behavior: "auto" });
    } else if (targetRect.top < topPadding) {
      window.scrollBy({ top: targetRect.top - topPadding, behavior: "auto" });
    } else if (targetRect.bottom > viewportHeight - bottomPadding) {
      window.scrollBy({ top: targetRect.bottom - viewportHeight + bottomPadding, behavior: "auto" });
    }
  };
...
function syncModeChoiceBoardStateClasses(board) {
  if (!board) {
    return;
  }

  const activePath = board.dataset.activePath || "";
  const openColumn = board.querySelector(".mode-choice-column.is-open");
  const openingColumn = board.querySelector(".mode-choice-column.is-opening, .mode-choice-column.is-targeting");
  const activeColumn = openColumn || openingColumn || null;
  board.classList.toggle("has-active-mode-column", Boolean(activePath || activeColumn));
  board.classList.toggle("has-open-mode-column", Boolean(openColumn));
  board.classList.toggle("has-opening-mode-column", Boolean(openingColumn));
  ["learn", "play", "train"].forEach((path) => {
    const columnForPath = board.querySelector(`.mode-choice-column-${path}.is-open, .mode-choice-column-${path}.is-opening, .mode-choice-column-${path}.is-targeting`);
    board.classList.toggle(`is-mode-path-${path}`, activePath === path || Boolean(columnForPath));
  });
}
...
function toggleModeChoiceMenu(button) {
  ...
  revealTouchModeChoiceMenu(column);
  ...
  board._modeChoiceTimer = window.setTimeout(() => {
    board.querySelectorAll(".mode-choice-column").forEach((item) => {
      if (item === column) {
        item.classList.remove("is-opening", "is-targeting");
        if (isTouchLandscapePresentationActive()) {
          applyTouchModeChoiceCardGrid(item);
          revealTouchModeChoiceMenu(item);
        } else {
          clearModeChoiceCardSpread(item);
        }
        clearModeChoiceColumnPosition(item);
...
function render() {
  ...
  syncPopupScrollLock();
  syncOrientationGateBackground(document.body.classList.contains("needs-landscape"));
}

function getDevicePresentationController() {
  if (!createDevicePresentationController) {
    return null;
  }
  if (!devicePresentationController) {
    devicePresentationController = createDevicePresentationController({
      window,
      document,
      refs,
      setHtml: setAppHtml,
  ...
  return devicePresentationController;
}

function setupDevicePresentationController() {
  getDevicePresentationController()?.setup();
}

function syncDevicePresentationState() {
  getDevicePresentationController()?.syncState();
}

function getDeviceViewport() {
  const controllerViewport = getDevicePresentationController()?.getViewport?.();
  if (controllerViewport) {
    return controllerViewport;
  }
  const width = Math.round(window.visualViewport?.width || window.innerWidth || document.documentElement.clientWidth || 1280);
  const height = Math.round(window.visualViewport?.height || window.innerHeight || document.documentElement.clientHeight || 800);
  return { width, height };
}

function syncOrientationGateBackground(isVisible) {
  getDevicePresentationController()?.syncGateBackground(isVisible);
}
...
function renderAppEntryGate() {
  ...
  if (!state.ui.appEntryGateOpen) {
    clearAppHtml(refs.appEntryGateMount);
    return;
  }
...
function renderWizard() {
  if (state.ui.appShellMode === "online") {
    ...
    if (refs.wizardRailMount) {
      clearAppHtml(refs.wizardRailMount);
    }
    if (isAlpacaOnlineCampusView() && refs.wizardSteps.querySelector("[data-campus2d-shell]")) {
      return;
    }
    setAppHtml(refs.wizardSteps, renderAlpacaOnlineHub(), "online-hub");
    return;
  }
  ...
  if (refs.wizardRailMount) {
    setAppHtml(refs.wizardRailMount, renderedWizard.railHtml, "wizard-rail");
  }

  setAppHtml(refs.wizardSteps, renderedWizard.stepsHtml, "wizard-steps");
}
...
function renderLiveOverlayMount() {
  ...
  const nextOverlay = parseAppFirstElement(html, "live-overlay");
  if (!nextOverlay) {
    clearAppHtml(mount);
    return;
  }
...
function renderLiveWaitingVideoRail(videos) {
  ...
  const canEmbedVideo = Boolean(embeddedVideo) && !IS_DESKTOP_APP;
...
function scrollExperiencePanelIntoView() {
  ...
  const scroll = () => {
    ...
    if (touchLandscape) {
      const topPadding = 8;
      const rect = refs.experiencePanel.getBoundingClientRect();
    ...
    refs.experiencePanel.scrollIntoView({ behavior, block: "start" });
  };
  scroll();
...
function scheduleCompactTouchFeedbackVisibility() {
  const viewport = getDeviceViewport();
  if (!document.body.classList.contains("is-touch-landscape") || viewport.height > 540) {
    return;
  }

  const reveal = () => revealCompactTouchFeedback();
  window.requestAnimationFrame(() => {
    reveal();
...
function revealCompactTouchFeedback() {
  const viewport = getDeviceViewport();
  if (!document.body.classList.contains("is-touch-landscape") || viewport.height > 540) {
    return;
  }

  const feedback = document.querySelector(".question-popup-window .feedback-card");
  const popup = feedback?.closest(".question-popup-window");
  if (!feedback || !popup) {
    return;
  }

  const target = feedback.querySelector("button:not([disabled])") || feedback;
  ...
  if (
    popupRect.width <= 0 ||
    popupRect.height <= 0 ||
    feedbackRect.width <= 0 ||
    feedbackRect.height <= 0 ||
    targetRect.width <= 0 ||
    targetRect.height <= 0
  ) {
    return;
  ...
  if (targetRect.top >= popupRect.top + topPadding && targetRect.bottom <= popupRect.bottom - bottomPadding) {
    return;
  }

  popup.scrollBy({
    top: targetRect.top - popupRect.top - topPadding,
    behavior: "auto"
  });
}

function scheduleCompactTouchPopupAnswerVisibility() {
  if (!document.body.classList.contains("is-touch-landscape")) {
    return;
  }

  const reveal = () => revealCompactTouchPopupAnswers();
  window.requestAnimationFrame(() => {
    reveal();
    window.setTimeout(reveal, 80);
  });
...
function revealCompactTouchPopupAnswers() {
  if (!document.body.classList.contains("is-touch-landscape")) {
    return;
  }

  const answer = document.querySelector([
    ".question-popup-window.relay [data-relay-option]:not([disabled])",
    ".question-popup-window.jump [data-jump-option]:not([disabled])"
  ].join(", "));
  const popup = answer?.closest(".question-popup-window");
  if (!answer || !popup) {
    return;
  }
  ...
  if (popupRect.width <= 0 || popupRect.height <= 0 || answerRect.width <= 0 || answerRect.height <= 0) {
    return;
  }

  const topPadding = Math.min(86, Math.max(16, popupRect.height * 0.22));
  const bottomPadding = 14;
  if (answerRect.top >= popupRect.top + 12 && answerRect.bottom <= popupRect.bottom - bottomPadding) {
    return;
  }

  popup.scrollBy({
    top: answerRect.top - popupRect.top - topPadding,
    behavior: "auto"
  });
}
...
function renderExperience() {
  ...
  if (state.ui.appShellMode === "online") {
    ...
    clearAppHtml(refs.experiencePanel);
  ...
  syncExperiencePanelTypeClass(state.experience.type);
...
function syncExperiencePanelTypeClass(type) {
  ...
  [...refs.experiencePanel.classList]
    .filter((className) => className.startsWith("experience-panel--"))
    .forEach((className) => refs.experiencePanel.classList.remove(className));

  if (type) {
    refs.experiencePanel.classList.add(`experience-panel--${type}`);
  }
}
...
function renderPreservingScroll(renderCallback) {
  ...
  window.requestAnimationFrame(() => {
    restoreScroll();
    window.requestAnimationFrame(() => restoreScroll(true));
  });
}

function capturePreservedElementScrolls() {
  return [
...
function restorePreservedElementScrolls(scrollStates) {
...
function syncRadialMindMapScroll() {
  ...
  window.requestAnimationFrame(() => {
    ...
    maps.forEach((map) => {
      ...
      stage.style.setProperty("--mindmap-touch-target-scale", Math.min(2.6, Math.max(1, 0.95 / scale)).toFixed(4));
...
function replaceMarkup(target, markup) {
  ...
  const nextNode = parseAppFirstElement(markup, "replace-markup");
  if (!nextNode) {
    return null;
  }
...
function syncPopupScrollLock() {
  ...
  const shouldLock = blockingOverlayOpen || hasActiveQuestionPopup();
...
function renderResourcesModal() {
  ...
  setAppHtml(refs.resourcesModalMount, state.ui.resourcesOpen ? `
...
function getAppSettingsController() {
  if (!createAppSettingsController) {
    return null;
  }
  if (!appSettingsController) {
    appSettingsController = createAppSettingsController({
      window,
      document,
      state,
      storageKey: APP_SETTINGS_STORAGE_KEY,
      backgroundMusicSrc: APP_BACKGROUND_MUSIC_SRC,
      defaultSettings: APP_DEFAULT_SETTINGS,
      setHtml: setAppHtml,
      clearHtml: clearAppHtml,
      escapeHtml,
      getAccountState: () => ({
...
function renderAppSettingsModal() {
  getAppSettingsController()?.renderModal();
}

function readSharedAppNumberSetting(value, fallback, min, max) {
  const numberSetting = getAppSettingsController()?.readNumberSetting?.(value, fallback, min, max);
...
function pauseAppBackgroundMusic() {
  getAppSettingsController()?.pauseBackgroundMusic();
}

function syncAppBackgroundMusicPlayback(options) {
...
function openAppSettingsPanel() {
  const controller = getAppSettingsController();
  if (controller) {
...
function toggleAppSettingsMute() {
  getAppSettingsController()?.toggleMute();
}
...
function renderLibraryCampusModal() {
  ...
  if (shouldRenderCampusLiveGamePanel()) {
    setAppHtml(mount, renderCampusLiveGamePanel(), "campus-live-game-panel");
    return;
  }
  if (state.ui.libraryExperience && state.experience) {
    setAppHtml(mount, renderLibraryExperienceViewer(), "library-experience-viewer");
    return;
  }

  if (state.ui.multiplayerGameChoice) {
    setAppHtml(mount, renderMultiplayerGameChoice(state.ui.multiplayerGameChoice), "multiplayer-game-choice");
    return;
  }

  if (state.ui.librarySectionPicker) {
    setAppHtml(mount, renderLibraryCampusSectionPicker(state.ui.librarySectionPicker), "library-section-picker");
    return;
  }

  if (state.ui.libraryResource) {
    setAppHtml(mount, renderLibraryResourceViewer(state.ui.libraryResource), "library-resource-viewer");
    return;
  }

  if (state.ui.libraryMenu) {
    setAppHtml(mount, renderLibraryCampusMenu(state.ui.libraryMenu.type), "library-campus-menu");
    return;
  }
...
function renderLibraryExperienceViewer() {
  ...
  const panelClasses = [
    ...
    state.experience?.type ? `experience-panel--${state.experience.type}` : ""
  ].filter(Boolean).join(" ");
...
function renderCooperationModal() {
  ...
  setAppHtml(refs.cooperationModalMount, state.ui.cooperationOpen ? `
...
function updateJumpDom(experience) {
  ...
  if (runner) {
    ...
    if (runner.dataset.jumpRunnerState !== runnerState) {
      ...
      setAppHtml(runner, renderJumpRunner(experience), "jump-runner");
    }
  ...
  if (obstacle && experience.obstacle) {
    ...
    if (obstacle.dataset.jumpObstacleKind !== obstacleKind) {
      ...
      setAppHtml(obstacle, renderJumpObstacle(experience.obstacle), "jump-obstacle");
    }
  ...
  if (lives) {
    setAppHtml(lives, renderJumpLives(experience.lives), "jump-lives");
  }
}
...
function getProgressStorageController() {
  if (!createProgressStorageController) {
    return null;
  }
  if (!progressStorageController) {
    progressStorageController = createProgressStorageController({
      storageService: appStorageService,
      progressService: appProgressService
    });
  }
  return progressStorageController;
}

function loadStats() {
  const defaults = getDefaultStats();
  const controller = getProgressStorageController();
  if (controller?.loadLocalProgress) {
    return controller.loadLocalProgress().stats;
  }
...
function loadRawMastery() {
  const controller = getProgressStorageController();
  if (controller?.loadLocalProgress) {
    return controller.loadLocalProgress().rawMastery;
  }
...
function saveProgressLocally() {
  ...
  try {
    ...
    // Private browsing and storage-quota failures should not interrupt practice.
  }
}
...
function setAppHtml(target, markup, source = "app-renderer") {
  if (!target) {
    return;
  }
  if (appDomService?.setHtml) {
    appDomService.setHtml(target, markup, source);
    return;
  }
  target.textContent = String(markup || "");
}

function clearAppHtml(target) {
  if (!target) {
    return;
  }
  if (appDomService?.clearHtml) {
    appDomService.clearHtml(target);
    return;
  }
  target.replaceChildren();
}

function parseAppFirstElement(markup, source = "app-renderer") {
  if (!appDomService?.parseTrustedFirstElement || !appDomService?.trustedHtml) {
    return null;
  }
  return appDomService.parseTrustedFirstElement(appDomService.trustedHtml(markup, source), document);
}

window.WSC_APP_RUNTIME_READY = true;
