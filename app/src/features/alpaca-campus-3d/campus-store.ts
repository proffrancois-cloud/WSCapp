...
const CHAT_MESSAGE_TTL_MS = 6200;
...
export const useCampusStore = create<CampusState>((set, get) => ({
  ...
  nudgeViewSettings(delta) {
    set((state) => ({
      viewSettings: {
        yaw: state.viewSettings.yaw + (delta.yaw || 0),
        distance: clampNumber(state.viewSettings.distance + (delta.distance || 0), 0.42, 2.45),
        height: clampNumber(state.viewSettings.height + (delta.height || 0), -5.05, 8.25),
